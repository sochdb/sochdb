...
use toondb_query::sql::{ExecutionResult, SqlError, SqlExecutor};
...
fn test_insert_null_values() {
    ...
    if let ExecutionResult::Rows { rows, .. } = select {
        ...
        let row = &rows[0];
        assert!(matches!(row.get("optional"), Some(ToonValue::Null)));
    }
}
...
fn test_insert_with_expressions() {
    ...
    if let ExecutionResult::Rows { rows, .. } = select {
        ...
        assert!(matches!(row.get("computed"), Some(ToonValue::Int(20))));
    }
}
...
fn test_select_with_where_equals() {
    ...
    if let ExecutionResult::Rows { rows, .. } = result {
        assert_eq!(rows.len(), 1);
        let row = &rows[0];
        assert!(matches!(row.get("name"), Some(ToonValue::Text(s)) if s == "Alice"));
    } else {
        panic!("Expected rows");
    }
}
...
fn test_select_with_functions() {
    ...
    if let ExecutionResult::Rows { rows, .. } = result {
        assert!(
            rows[0]
                .values()
                .any(|v| matches!(v, ToonValue::Text(s) if s == "ALICE"))
        );
    }
}
...
fn test_select_with_lower_function() {
    ...
    if let ExecutionResult::Rows { rows, .. } = result {
        assert!(
            rows[0]
                .values()
                .any(|v| matches!(v, ToonValue::Text(s) if s == "hello world"))
        );
    }
}
...
fn test_select_with_length_function() {
    ...
    if let ExecutionResult::Rows { rows, .. } = result {
        assert!(rows[0].values().any(|v| matches!(v, ToonValue::Int(5))));
    }
}
...
fn test_select_with_arithmetic() {
    ...
    if let ExecutionResult::Rows { rows, .. } = result {
        ...
        if let ToonValue::Float(f) = value {
...
fn test_update_single_row() {
    ...
    if let ExecutionResult::Rows { rows, .. } = select {
        assert!(matches!(rows[0].get("age"), Some(ToonValue::Int(31))));
    }
}
...
fn test_update_multiple_columns() {
    ...
    if let ExecutionResult::Rows { rows, .. } = select {
        assert!(matches!(rows[0].get("name"), Some(ToonValue::Text(s)) if s == "Alicia"));
        assert!(matches!(rows[0].get("age"), Some(ToonValue::Int(31))));
    }
}
...
fn test_update_with_expression() {
    ...
    let select = fixture.exec("SELECT age FROM users WHERE id = 1").unwrap();
    if let ExecutionResult::Rows { rows, .. } = select {
        assert!(matches!(rows[0].get("age"), Some(ToonValue::Int(31)))); // Was 30
    }
}
...
fn test_workflow_crud_operations() {
    ...
    let result = fixture
        ...
        .unwrap();
    if let ExecutionResult::Rows { rows, .. } = result {
        assert!(matches!(rows[0].get("quantity"), Some(ToonValue::Int(75))));
    }
