...
use toondb_query::context_query::{
...
use toondb_query::toon_ql::ToonValue;
...
fn test_full_query_parse_and_validate() {
    ...
    assert_eq!(result.options.format, OutputFormat::Toon);
...
fn test_vector_index_metadata_preservation() {
    ...
    metadata.insert(
        "source".to_string(),
        ToonValue::Text("wikipedia".to_string()),
    );
    metadata.insert("page".to_string(), ToonValue::Int(42));
    metadata.insert("confidence".to_string(), ToonValue::Float(0.95));
    ...
    match result.metadata.get("source") {
        Some(ToonValue::Text(s)) => assert_eq!(s, "wikipedia"),
