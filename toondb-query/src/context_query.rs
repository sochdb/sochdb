...
use crate::toon_ql::{ComparisonOp, Condition, LogicalOp, ToonValue, WhereClause};
...
/// or wrapped around the toondb-index HNSW implementation.
...
struct VectorCollection {
    ...
    vectors: Vec<(String, Vec<f32>, String, HashMap<String, ToonValue>)>,
    /// Vector dimension
    dimension: usize,
}
...
/// Production-ready vector index backed by HNSW from toondb-index.
/// 
...
struct HnswCollection {
    ...
    index: toondb_index::vector::VectorIndex,
    /// Metadata storage: edge_id -> (id, content, metadata)
    #[allow(clippy::type_complexity)]
    metadata: HashMap<u128, (String, String, HashMap<String, ToonValue>)>,
...
impl HnswVectorIndex {
    ...
    pub fn create_collection(&self, name: &str, dimension: usize) {
        ...
        collections.entry(name.to_string()).or_insert_with(|| {
            let index = toondb_index::vector::VectorIndex::with_dimension(
                toondb_index::vector::DistanceMetric::Cosine,
                dimension,
            );
            HnswCollection {
                index,
...
impl ContextQueryParser {
    ...
    /// Parse a value
    fn parse_value(&mut self) -> Result<ToonValue, ContextParseError> {
        match self.current().clone() {
            Token::Number(n) => {
                self.advance();
                if n.fract() == 0.0 {
                    Ok(ToonValue::Int(n as i64))
                } else {
                    Ok(ToonValue::Float(n))
                }
            }
            Token::String(s) => {
                self.advance();
...
mod tests {
    ...
    fn test_vector_index_with_metadata() {
        ...
        metadata.insert("author".to_string(), ToonValue::Text("Alice".to_string()));
        metadata.insert("year".to_string(), ToonValue::Int(2024));
