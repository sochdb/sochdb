...
impl TokenEstimator {
    ...
    pub fn estimate_value(&self, value: &ToonValue) -> usize {
        match value {
            ToonValue::Null => 1,
            ToonValue::Bool(_) => 1, // "true" or "false" is typically 1 token
            ToonValue::Int(n) => {
                // Count digits + sign
                let digits = if *n == 0 {
                    1
    ...
    pub fn estimate_row(&self, values: &[ToonValue]) -> usize {
        if values.is_empty() {
            return 0;
        }
...
mod tests {
    ...
    fn test_estimate_value_int() {
        ...
        // Small integers
        assert!(est.estimate_value(&ToonValue::Int(0)) >= 1);
        assert!(est.estimate_value(&ToonValue::Int(42)) >= 1);

        // Large integers use more tokens
        let small = est.estimate_value(&ToonValue::Int(42));
        let large = est.estimate_value(&ToonValue::Int(1_000_000_000));
        assert!(large >= small);
    }
    ...
    fn test_estimate_value_text() {
        ...
        let short = est.estimate_value(&ToonValue::Text("hello".to_string()));
        let long = est.estimate_value(&ToonValue::Text(
    ...
    fn test_estimate_table() {
        ...
        let rows = vec![
            vec![ToonValue::Int(1), ToonValue::Text("Alice".to_string())],
            vec![ToonValue::Int(2), ToonValue::Text("Bob".to_string())],
        ];
    ...
    fn test_truncate_rows() {
        ...
        let rows: Vec<Vec<ToonValue>> = (0..100)
            .map(|i| vec![ToonValue::Int(i), ToonValue::Text(format!("row{}", i))])
            .collect();
