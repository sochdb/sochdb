...
pub struct RowBuilder<'a> {
    ...
    current: HashMap<String, ToonValue>,
}

impl<'a> RowBuilder<'a> {
    /// Create new row builder
    pub fn new(conn: &'a ToonConnection, table: &str) -> Self {
        Self {
            conn,
    ...
    /// Set field value
    pub fn set(mut self, field: &str, value: impl Into<ToonValue>) -> Self {
...
pub struct UpdateBuilder<'a> {
    conn: &'a ToonConnection,
    table: String,
    updates: HashMap<String, ToonValue>,
...
impl<'a> UpdateBuilder<'a> {
    ...
    /// Add WHERE clause
    pub fn where_eq(mut self, field: &str, value: impl Into<ToonValue>) -> Self {
    ...
    pub fn where_cond(mut self, field: &str, op: CompareOp, value: impl Into<ToonValue>) -> Self {
        self.where_clause = Some(WhereClause::Simple {
            field: field.to_string(),
            op,
            value: value.into(),
...
impl<'a> FindBuilder<'a> {
    ...
    pub fn execute(self) -> Result<Vec<HashMap<String, ToonValue>>> {
    ...
    pub fn first(mut self) -> Result<Option<HashMap<String, ToonValue>>> {
        self.limit = Some(1);
    ...
    pub fn all(self) -> Result<Vec<HashMap<String, ToonValue>>> {
        self.execute()
    }
}
...
impl<'a> UpsertBuilder<'a> {
    ...
    pub fn new(conn: &'a ToonConnection, table: &str, conflict_key: &str) -> Self {
        Self {
            conn,
            table: table.to_string(),
...
impl ToonConnection {
    ...
    /// Check if row exists
    pub fn exists(&self, table: &str, field: &str, value: impl Into<ToonValue>) -> Result<bool> {
...
impl<'a> ClientTransaction<'a> {
    ...
    pub fn insert(&mut self, table: &str, values: HashMap<String, ToonValue>) -> Result<u64> {
        let mut tch = self.conn.tch.write();
...
mod tests {
    ...
    fn test_insert_builder() {
        ...
        let result = conn
            .insert_into("users")
            .set("id", ToonValue::Int(1))
            .set("name", ToonValue::Text("Alice".to_string()))
            .execute()
            .unwrap();
    ...
    fn test_multi_row_insert() {
        ...
        let result = conn
            ...
            .set("name", ToonValue::Text("Alice".to_string()))
            .row()
            .set("id", ToonValue::Int(2))
            .set("name", ToonValue::Text("Bob".to_string()))
            .execute()
            .unwrap();
    ...
    fn test_update_builder() {
        ...
        let result = conn
            .update("users")
            .set("name", ToonValue::Text("New Name".to_string()))
            .where_eq("id", ToonValue::Int(1))
            .execute()
            .unwrap();
    ...
    fn test_find_builder() {
        ...
        let result = conn
            .find("users")
            .select(&["id", "name"])
            .where_eq("active", ToonValue::Bool(true))
            .order_by("name", true)
            .limit(10)
            .execute();
    ...
    fn test_convenience_methods() {
        ...
        // Quick find
        let _user = conn.find_by_id("users", "id", ToonValue::Int(1));

        // Exists check
        let _exists = conn.exists(
            "users",
            "email",
            ToonValue::Text("test@test.com".to_string()),
        );
