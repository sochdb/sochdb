...
// Import SIMD-optimized distance functions from toondb-index
use toondb_index::vector_simd::l2_squared_f32;
...
impl VectorCollection {
    ...
    pub fn open(conn: &Arc<ToonConnection>, name: &str) -> Result<Self> {
    ...
    pub fn create(conn: &Arc<ToonConnection>, name: &str, dimension: usize) -> Result<Self> {
        Ok(Self {
            name: name.to_string(),
            dimension,
            backend: VectorBackend::InMemory {
...
/// Euclidean distance using SIMD-accelerated L2 squared from toondb-index
///
...
mod tests {
    ...
    fn test_conn() -> Arc<ToonConnection> {
        Arc::new(ToonConnection::open("./test").unwrap())
    }
