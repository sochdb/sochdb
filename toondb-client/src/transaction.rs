...
use crate::connection::{Timestamp, ToonConnection, TxnId};
...
pub struct ClientTransaction<'a> {
    pub(crate) conn: &'a ToonConnection,
    /// Transaction ID
    txn_id: TxnId,
    /// Start timestamp
    start_ts: Timestamp,
...
impl<'a> ClientTransaction<'a> {
    ...
    pub fn begin(conn: &'a ToonConnection, isolation: IsolationLevel) -> Result<Self> {
...
impl<'a> SnapshotReader<'a> {
    ...
    pub fn now(conn: &'a ToonConnection) -> Result<Self> {
    ...
    pub fn at_timestamp(conn: &'a ToonConnection, ts: Timestamp) -> Result<Self> {
        Ok(Self {
            conn,
            snapshot_ts: ts,
            track_visibility: false,
