...
impl<'a> BatchWriter<'a> {
            ...
            for op in batch_ops {
                match op {
                    BatchOp::Insert { table, values } => {
                        let map: std::collections::HashMap<String, ToonValue> =
    ...
    pub fn insert(mut self, table: &str, values: Vec<(&str, ToonValue)>) -> Self {
    ...
    pub fn delete(mut self, table: &str, key_field: &str, key_value: ToonValue) -> Self {
...
///
/// **DEPRECATED**: Use `toondb_storage::GroupCommitBuffer` instead.
...
#[deprecated(
    since = "0.2.0",
    note = "Use toondb_storage::EventDrivenGroupCommit for actual WAL integration"
)]
...
impl ToonConnection {
    ...
    pub fn bulk_insert_slice(
        &self,
        table: &str,
        rows: Vec<(u64, Vec<Option<ToonValue>>)>,
    ) -> Result<BatchResult> {
        let mut batch = BatchWriter::new(self)
            .max_batch_size(1000)
            .auto_commit(true);
...
mod tests {
    ...
    fn test_batch_writer() {
        ...
        let result = conn
            ...
            .insert(
                ...
                vec![
                    ("id", ToonValue::Int(1)),
                    ("name", ToonValue::Text("Alice".to_string())),
                ],
            )
            .insert(
                "users",
                vec![
                    ("id", ToonValue::Int(2)),
                    ("name", ToonValue::Text("Bob".to_string())),
                ],
            )
            .execute()
            .unwrap();
    ...
    fn test_streaming_batch_writer() {
        let conn = ToonConnection::open("./test_streaming").unwrap();
        ...
        let result = conn
            ...
            .max_batch_size(2)
            .auto_commit(true)
            .insert("users", vec![("id", ToonValue::Int(1))])
            .insert("users", vec![("id", ToonValue::Int(2))])
            .insert("users", vec![("id", ToonValue::Int(3))])
            .insert("users", vec![("id", ToonValue::Int(4))])
            .insert("users", vec![("id", ToonValue::Int(5))])
            .execute()
            .unwrap();
    ...
    fn test_bulk_insert() {
        ...
        let rows = vec![
            vec![
                ("id", ToonValue::Int(1)),
                ("name", ToonValue::Text("A".to_string())),
            ],
            vec![
                ("id", ToonValue::Int(2)),
                ("name", ToonValue::Text("B".to_string())),
            ],
            vec![
                ("id", ToonValue::Int(3)),
                ("name", ToonValue::Text("C".to_string())),
            ],
        ];
    ...
    fn test_insert_slice() {
        ...
        let conn = ToonConnection::open("./test_slice").unwrap();
        ...
        let result = conn
            ...
            .insert_slice(
                ...
                2,
                vec![
                    Some(ToonValue::UInt(2)),
                    Some(ToonValue::Text("Bob".to_string())),
                ],
            )
            .execute()
            .unwrap();
