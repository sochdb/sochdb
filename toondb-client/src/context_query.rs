...
//! CONTEXT SELECT queries from the ToonDB client SDK.
//!
//! ## Usage
//!
//! ```rust,ignore
//! use toondb::context_query::ContextQueryBuilder;
//!
...
use crate::connection::{CompareOp, ToonConnection, WhereClause};
use toondb_core::toon::ToonValue;
...
pub struct ContextQueryBuilder {
    ...
    connection: Option<Arc<ToonConnection>>,
}
...
impl ContextQueryBuilder {
    ...
    pub fn with_connection(conn: Arc<ToonConnection>) -> Self {
        Self {
            session_id: None,
            agent_id: None,
            token_budget: 4096,
    ...
    pub fn connection(mut self, conn: Arc<ToonConnection>) -> Self {
...
impl ContextQuery {
    ...
    /// - Parsed by ToonQL
    pub fn to_canonical(&self) -> toondb_query::context_query::ContextSelectQuery {
        use toondb_query::context_query as cq;
        ...
        let format = match self.format {
            ContextFormat::Toon => cq::OutputFormat::Toon,
            ContextFormat::Json => cq::OutputFormat::Json,
            ContextFormat::Markdown => cq::OutputFormat::Markdown,
            ContextFormat::Text => cq::OutputFormat::Toon, // Map plain text to TOON
        };
    ...
    fn convert_section(&self, section: &ContextSection) -> toondb_query::context_query::ContextSection {
        use toondb_query::context_query as cq;
    ...
    fn convert_filter(&self, filter: &FilterExpr) -> toondb_query::toon_ql::WhereClause {
        use toondb_query::toon_ql as tq;
        ...
        let (conditions, operator) = match filter {
            FilterExpr::Eq(col, val) => (
                vec![tq::Condition {
                    ...
                    value: tq::ToonValue::Text(val.clone()),
                }],
                tq::LogicalOp::And,
            ),
            ...
            FilterExpr::Like(col, pat) => (
                vec![tq::Condition {
                    ...
                    value: tq::ToonValue::Text(pat.clone()),
                }],
                tq::LogicalOp::And,
            ),
            FilterExpr::In(col, vals) => (
                vec![tq::Condition {
                    ...
                    value: tq::ToonValue::Array(vals.iter().map(|v| tq::ToonValue::Text(v.clone())).collect()),
                }],
                tq::LogicalOp::And,
            ),
    ...
    fn filter_to_where(&self, filter: &FilterExpr) -> WhereClause {
        match filter {
            FilterExpr::Eq(col, val) => WhereClause::Simple {
                field: col.clone(),
                op: CompareOp::Eq,
                value: ToonValue::Text(val.clone()),
            },
            FilterExpr::Gt(col, val) => WhereClause::Simple {
                field: col.clone(),
                op: CompareOp::Gt,
                value: ToonValue::Int(*val),
            },
            ...
            FilterExpr::In(col, vals) => {
                ...
                WhereClause::In {
                    field: col.clone(),
                    values: vals.iter().map(|v| ToonValue::Text(v.clone())).collect(),
                    negated: false,
                }
            }
    ...
    fn format_cursor_as_toon(
        ...
        section_name: &str,
        table: &str,
        mut cursor: crate::connection::ToonCursor,
    ) -> Result<String, ContextQueryError> {
            ...
            ContextFormat::Toon => {
                ...
                for row in &rows {
                    let values: Vec<String> = fields
                        .iter()
                        .filter_map(|f| row.get(*f).map(format_toon_value))
                        .collect();
                    output.push_str(&format!("  [{}]\n", values.join(", ")));
                }
            }
...
// ============================================================================
// Token Estimation (delegate to toondb-query)
// ============================================================================

use toondb_query::token_budget::TokenEstimator;
...
/// Format a ToonValue for TOON output
fn format_toon_value(v: &ToonValue) -> String {
    match v {
        ...
        ToonValue::Binary(b) => format!("b64:<{}bytes>", b.len()),
        ToonValue::Array(arr) => {
            let items: Vec<String> = arr.iter().map(format_toon_value).collect();
            format!("[{}]", items.join(","))
        }
        ...
        ToonValue::Ref { table, id } => format!("ref({},{})", table, id),
    }
}

/// Format a ToonValue for JSON output
fn format_json_value(v: &ToonValue) -> String {
    match v {
        ToonValue::Null => "null".to_string(),
        ToonValue::Int(i) => i.to_string(),
        ToonValue::UInt(u) => u.to_string(),
        ToonValue::Float(f) => format!("{}", f),
        ToonValue::Text(s) => format!("\"{}\"", s.replace('\\', "\\\\").replace('"', "\\\"")),
        ToonValue::Bool(b) => if *b { "true" } else { "false" }.to_string(),
        ToonValue::Binary(b) => format!("\"<binary:{}>\"", b.len()),
        ToonValue::Array(arr) => {
        ...
        ToonValue::Ref { table, id } => format!("{{\"$ref\":\"{}\",\"id\":{}}}", table, id),
    }
}
