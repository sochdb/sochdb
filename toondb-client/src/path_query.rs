...
use crate::connection::{PathResolution, ToonConnection};
use crate::error::{ClientError, Result};
use crate::result::ToonResult;
...
impl ColumnPredicate {
    pub fn new(field: &str, op: CompareOp, value: ToonValue) -> Self {
        Self {
    ...
    pub fn evaluate(&self, value: &ToonValue) -> bool {
        match self.op {
            ...
            CompareOp::IsNull => matches!(value, ToonValue::Null),
            CompareOp::IsNotNull => !matches!(value, ToonValue::Null),
        }
    }

    fn compare_ord(&self, value: &ToonValue) -> Option<std::cmp::Ordering> {
    ...
    fn match_like(&self, value: &ToonValue) -> bool {
        match (value, &self.value) {
            (ToonValue::Text(text), ToonValue::Text(pattern)) => {
    ...
    fn match_in(&self, value: &ToonValue) -> bool {
        match &self.value {
            ToonValue::Array(values) => values.contains(value),
            _ => false,
        }
    }
}
...
impl<'a> PathQuery<'a> {
    ...
    pub fn filter(mut self, field: &str, op: CompareOp, value: impl Into<ToonValue>) -> Self {
        self.predicates
    ...
    pub fn where_like(self, field: &str, pattern: &str) -> Self {
        self.filter(field, CompareOp::Like, ToonValue::Text(pattern.to_string()))
    }
...
impl Predicate {
    pub fn eq(column: &str, value: ToonValue) -> Self {
        Self {
    ...
    pub fn gt(column: &str, value: ToonValue) -> Self {
        Self {
            column: column.to_string(),
            op: CompareOp::Gt,
            value,
    ...
    pub fn lt(column: &str, value: ToonValue) -> Self {
        Self {
            column: column.to_string(),
            op: CompareOp::Lt,
            value,
    ...
    pub fn like(column: &str, pattern: &str) -> Self {
        Self {
            column: column.to_string(),
            op: CompareOp::Like,
            value: ToonValue::Text(pattern.to_string()),
        }
    }
}
...
mod tests {
    ...
    fn test_path_query_basic() {
        let conn = ToonConnection::open("./test").unwrap();
    ...
    fn test_path_query_filter() {
        ...
        let query = PathQuery::from_path(&conn, "users")
            .where_gt("score", ToonValue::Float(80.0))
            .limit(10);
    ...
    fn test_predicate_evaluate() {
        let pred = ColumnPredicate::new("score", CompareOp::Gt, ToonValue::Int(80));

        assert!(pred.evaluate(&ToonValue::Int(90)));
        assert!(!pred.evaluate(&ToonValue::Int(70)));
        assert!(!pred.evaluate(&ToonValue::Int(80)));
    }
    ...
    fn test_predicate_like() {
        let pred = ColumnPredicate::new(
            "name",
            CompareOp::Like,
            ToonValue::Text("John%".to_string()),
        );

        assert!(pred.evaluate(&ToonValue::Text("John".to_string())));
        assert!(pred.evaluate(&ToonValue::Text("Johnny".to_string())));
        assert!(!pred.evaluate(&ToonValue::Text("Jane".to_string())));
    }
