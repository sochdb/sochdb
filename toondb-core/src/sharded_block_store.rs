...
use crate::block_storage::{
    BlockCompression, BlockHeader, BlockRef, is_compressible, is_json_content, is_toon_content,
};
        ...
        match compression {
            ...
            BlockCompression::Lz4 => {
                ...
                lz4::block::decompress(data, Some(original_size as i32))
                    .map_err(|e| ToonDBError::Corruption(format!("LZ4 decompress failed: {}", e)))
            }
            BlockCompression::Zstd => {
                ...
                zstd::decode_all(data)
                    .map_err(|e| ToonDBError::Corruption(format!("ZSTD decompress failed: {}", e)))
            }
        }
    }
}
...
impl ShardedBlockRef {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        if data.len() < 25 {
            return Err(ToonDBError::Corruption("ShardedBlockRef too short".into()));
        }
