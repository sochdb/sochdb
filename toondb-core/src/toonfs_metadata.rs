...
impl From<&Inode> for InodeRow {
    fn from(inode: &Inode) -> Self {
        Self {
            ...
            toon_schema: inode.toon_schema.clone(),
        }
    }
}
...
impl InodeRow {
    pub fn to_inode(&self) -> Inode {
        Inode {
            ...
            toon_schema: self.toon_schema.clone(),
        }
    }
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        bincode::deserialize(data).map_err(|e| ToonDBError::Serialization(e.to_string()))
    }
}
...
impl FsMetadataStore {
    ...
    pub fn create_inode(&self, file_type: FileType) -> Result<Inode> {
        ...
        let inode = match file_type {
            ...
            FileType::ToonDocument => Inode::new_toon(id, String::new()),
        };
    ...
    pub fn resolve_path(&self, path: &str) -> Result<InodeId> {
        ...
        for component in path.split('/') {
            ...
            current = self.lookup(current, component).ok_or_else(|| {
                ToonDBError::NotFound(format!("Path component not found: {}", component))
            })?;
        }
    ...
    pub fn create_file(&self, parent_id: InodeId, name: &str) -> Result<Inode> {
        ...
        let parent = self
            .get_inode(parent_id)
            .ok_or_else(|| ToonDBError::NotFound("Parent not found".into()))?;

        if !parent.is_dir() {
            return Err(ToonDBError::InvalidArgument(
        ...
        if self.lookup(parent_id, name).is_some() {
            return Err(ToonDBError::InvalidArgument("File already exists".into()));
        }
    ...
    pub fn delete(&self, parent_id: InodeId, name: &str) -> Result<()> {
        let child_id = self
            ...
            .ok_or_else(|| ToonDBError::NotFound("Entry not found".into()))?;

        let child = self
            .get_inode(child_id)
            .ok_or_else(|| ToonDBError::NotFound("Inode not found".into()))?;

        // If directory, must be empty
        if child.is_dir() {
            let entries = self.list_dir(child_id);
            if !entries.is_empty() {
                return Err(ToonDBError::InvalidArgument("Directory not empty".into()));
            }
        }
...
/// ToonFS - Complete filesystem layer
#[allow(clippy::type_complexity)]
pub struct ToonFS {
    /// Metadata store
...
impl ToonFS {
    ...
    pub fn readdir(&self, path: &str) -> Result<Vec<DirEntry>> {
        ...
        let inode = self
            .metadata
            .get_inode(inode_id)
            .ok_or_else(|| ToonDBError::NotFound("Directory not found".into()))?;

        if !inode.is_dir() {
            return Err(ToonDBError::InvalidArgument("Not a directory".into()));
        }
    ...
    pub fn write_file(&self, path: &str, data: &[u8]) -> Result<usize> {
        ...
        let mut inode = self
            .metadata
            .get_inode(inode_id)
            .ok_or_else(|| ToonDBError::NotFound("File not found".into()))?;

        if !inode.is_file() && !inode.is_toon() {
            return Err(ToonDBError::InvalidArgument("Not a regular file".into()));
        }
...
mod tests {
    ...
    fn test_toonfs() {
        ...
        let fs = ToonFS::new(
            ...
            move |id, _size| {
                blocks_read
                    ...
                    .get(&id)
                    .cloned()
                    .ok_or_else(|| ToonDBError::NotFound("Block not found".into()))
            },
        );
        ...
        fs.write_file("/test.txt", b"Hello, ToonFS!").unwrap();

        // Read back
        let data = fs.read_file("/test.txt").unwrap();
        assert_eq!(data, b"Hello, ToonFS!");
