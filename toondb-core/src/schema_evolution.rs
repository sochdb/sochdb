...
//!
//! This module provides backwards-compatible schema evolution for ToonDB,
...
use crate::error::{Result, ToonDBError};
use crate::toon::{ToonRow, ToonSchema, ToonType, ToonValue};
...
impl TypeConverter {
    ...
    pub fn convert(
        &self,
        value: &ToonValue,
        custom_converters: &[fn(&ToonValue) -> ToonValue],
    ) -> Result<ToonValue> {
        match self {
            TypeConverter::Identity => Ok(value.clone()),
            TypeConverter::IntToText => match value {
                ToonValue::Int(i) => Ok(ToonValue::Text(i.to_string())),
                ToonValue::UInt(u) => Ok(ToonValue::Text(u.to_string())),
                _ => Err(ToonDBError::SchemaEvolution(format!(
                    "Cannot convert {:?} to text via IntToText",
                    value
                ))),
            },
            TypeConverter::TextToInt => match value {
                ToonValue::Text(s) => s.parse::<i64>().map(ToonValue::Int).map_err(|_| {
                    ToonDBError::SchemaEvolution(format!("Cannot parse '{}' as integer", s))
                }),
                _ => Err(ToonDBError::SchemaEvolution(format!(
            ...
            TypeConverter::FloatToInt => match value {
                ToonValue::Float(f) => Ok(ToonValue::Int(*f as i64)),
                _ => Err(ToonDBError::SchemaEvolution(format!(
            ...
            TypeConverter::IntToFloat => match value {
                ToonValue::Int(i) => Ok(ToonValue::Float(*i as f64)),
                ToonValue::UInt(u) => Ok(ToonValue::Float(*u as f64)),
                _ => Err(ToonDBError::SchemaEvolution(format!(
            ...
            TypeConverter::Custom(idx) => {
                if *idx < custom_converters.len() {
                    ...
                    Err(ToonDBError::SchemaEvolution(format!(
...
impl VersionedRow {
    pub fn new(version: SchemaVersion, data: ToonRow) -> Self {
        Self { version, data }
    }
}
...
pub struct SchemaRegistry {
    ...
    schemas: HashMap<SchemaId, ToonSchema>,
    ...
    custom_converters: Vec<fn(&ToonValue) -> ToonValue>,
    /// Statistics
...
impl SchemaRegistry {
    ...
    pub fn register_schema(&mut self, schema: ToonSchema, version: SchemaVersion) {
    ...
    pub fn register_migration(
        ...
        if self.migrations.contains_key(&key) {
            return Err(ToonDBError::SchemaEvolution(format!(
    ...
    pub fn register_converter(&mut self, converter: fn(&ToonValue) -> ToonValue) -> usize {
    ...
    pub fn get_schema(&self, name: &str, version: SchemaVersion) -> Option<&ToonSchema> {
        self.schemas.get(&SchemaId::new(name, version))
    }

    /// Get current schema by name
    pub fn current_schema(&self, name: &str) -> Option<&ToonSchema> {
    ...
    pub fn migrate_row(&self, name: &str, row: VersionedRow) -> Result<VersionedRow> {
        let current_version = self.current_version(name).ok_or_else(|| {
            ToonDBError::SchemaEvolution(format!("No schema registered for '{}'", name))
        })?;
        ...
        while version < current_version {
            let migration = self
                ...
                .ok_or_else(|| {
                    ToonDBError::SchemaEvolution(format!(
...
impl SchemaEvolutionManager {
    ...
    pub fn evolve_schema(
        ...
        let current_schema = self
            ...
            .current_schema(name)
            .ok_or_else(|| {
                ToonDBError::SchemaEvolution(format!("Current schema not found for '{}'", name))
            })?
            .clone();
        ...
        for change in changes {
            match change {
                SchemaChange::AddColumn {
                    ...
                    let field = crate::toon::ToonField {
                        name: name.clone(),
...
mod tests {
    ...
    fn test_add_column_migration() {
        ...
        let schema_v2 = ToonSchema::new("users")
            .field("id", ToonType::UInt)
            .field("name", ToonType::Text)
        ...
        // Register migration
        let migration =
            Migration::new(1, 2).add_column("email", ToonType::Text, ToonValue::Text("".into()));
        registry.register_migration("users", migration).unwrap();

        // Create v1 row
        let row_v1 = VersionedRow::new(
            1,
            ToonRow::new(vec![ToonValue::UInt(1), ToonValue::Text("Alice".into())]),
        );
        ...
        assert_eq!(row_v2.data.values[2], ToonValue::Text("".into()));
    }
    ...
    fn test_drop_column_migration() {
        ...
        let schema_v1 = ToonSchema::new("users")
            .field("id", ToonType::UInt)
            .field("name", ToonType::Text)
            .field("legacy", ToonType::Text);
        registry.register_schema(schema_v1, 1);
    ...
    fn test_rename_column_migration() {
        ...
        assert_eq!(row_v2.version, 2);
        assert_eq!(row_v2.data.values.len(), 2);
        assert_eq!(row_v2.data.values[1], ToonValue::Text("Alice".into()));
    }
    ...
    fn test_type_conversion_migration() {
        ...
        let schema_v1 = ToonSchema::new("products")
            .field("id", ToonType::UInt)
            .field("price", ToonType::Int);
        registry.register_schema(schema_v1, 1);

        let schema_v2 = ToonSchema::new("products")
            .field("id", ToonType::UInt)
            .field("price", ToonType::Float);
        registry.register_schema(schema_v2, 2);

        let migration =
            Migration::new(1, 2).change_type("price", ToonType::Float, TypeConverter::IntToFloat);
        registry.register_migration("products", migration).unwrap();

        let row_v1 = VersionedRow::new(
            1,
            ToonRow::new(vec![ToonValue::UInt(1), ToonValue::Int(100)]),
        );

        let row_v2 = registry.migrate_row("products", row_v1).unwrap();

        assert_eq!(row_v2.version, 2);
        assert_eq!(row_v2.data.values[1], ToonValue::Float(100.0));
    }
    ...
    fn test_multi_version_migration_chain() {
        ...
        let schema_v3 = ToonSchema::new("users")
            .field("id", ToonType::UInt)
            .field("full_name", ToonType::Text)
            .field("email", ToonType::Text)
            .field("created_at", ToonType::Int);
        ...
        let migration_2_3 = Migration::new(2, 3)
            ...
            .add_column("created_at", ToonType::Int, ToonValue::Int(0));
        ...
        assert_eq!(row_v3.data.values[0], ToonValue::UInt(1));
        assert_eq!(row_v3.data.values[1], ToonValue::Text("Alice".into()));
        assert_eq!(row_v3.data.values[2], ToonValue::Text("".into()));
        assert_eq!(row_v3.data.values[3], ToonValue::Int(0));
    }
    ...
    fn test_evolve_schema() {
        ...
        let changes = vec![SchemaChange::AddColumn {
            ...
            column_type: ToonType::Text,
            default: ToonValue::Text("".into()),
            position: None,
        }];
    ...
    fn test_custom_type_converter() {
        ...
        let converter_idx = registry.register_converter(|v| {
            if let ToonValue::Text(s) = v {
                ToonValue::Text(s.to_uppercase())
            } else {
                v.clone()
            }
        });
        ...
        let schema_v2 = ToonSchema::new("users")
            .field("id", ToonType::UInt)
            .field("name", ToonType::Text); // Same type, different format
        registry.register_schema(schema_v2, 2);
