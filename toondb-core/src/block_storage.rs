...
//!
//! Integrates PayloadStore as ToonFS data block backend:
...
//!
//! All serialization methods return `Result<T, ToonDBError>` instead of
...
impl BlockRef {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        if data.len() < Self::SERIALIZED_SIZE {
            return Err(ToonDBError::Serialization(format!(
                "BlockRef too short: {} < {}",
                data.len(),
...
impl BlockHeader {
    ...
    pub fn from_bytes(buf: &[u8]) -> Result<Self> {
        if buf.len() < Self::SIZE {
            return Err(ToonDBError::Corruption(format!(
    ...
    pub fn read_block(&self, block_ref: &BlockRef) -> Result<Vec<u8>> {
        ...
        let compressed = {
            ...
            if offset + total_size > store.len() {
                return Err(ToonDBError::NotFound("Block not found".into()));
            }
        ...
        if checksum != block_ref.checksum {
            return Err(ToonDBError::Corruption("Block checksum mismatch".into()));
        }
        ...
        match compression {
            ...
            BlockCompression::Lz4 => {
                ...
                lz4::block::decompress(data, Some(original_size as i32)).map_err(|e| {
                    ToonDBError::Corruption(format!("LZ4 decompression failed: {}", e))
                })
            }
            BlockCompression::Zstd => {
                // If data matches original size, it's uncompressed (fallback case)
                ...
                zstd::decode_all(data).map_err(|e| {
                    ToonDBError::Corruption(format!("ZSTD decompression failed: {}", e))
                })
            }
        }
    }
...
pub fn is_toon_content(data: &[u8]) -> bool {
...
impl WalRecordHeader {
    ...
    pub fn from_bytes(buf: &[u8]) -> Result<Self> {
        ...
        let record_type = WalRecordType::from_byte(buf[16]).ok_or_else(|| {
            ToonDBError::Corruption(format!("Invalid WAL record type: {}", buf[16]))
        })?;
...
mod tests {
    ...
    fn test_block_store_write_read() {
        ...
        let data = b"Hello, ToonFS block storage!";
    ...
    fn test_automatic_compression_selection() {
        ...
        let mut toon_data = vec![0u8; 256];
        toon_data[0..4].copy_from_slice(b"TOON"); // Magic header
        let toon_ref = store.write_block(&toon_data).unwrap();
        let toon_recovered = store.read_block(&toon_ref).unwrap();
        assert_eq!(toon_recovered, toon_data);
    }
