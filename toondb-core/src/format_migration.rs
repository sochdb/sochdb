...
//!
//! This module provides format versioning for ToonDB's block storage,
...
impl V2Header {
    ...
    pub fn from_bytes(buf: &[u8]) -> Result<Self> {
        ...
        let format_version = FormatVersion::from_byte(buf[4]).ok_or_else(|| {
            ToonDBError::InvalidData(format!("Unknown format version: {}", buf[4]))
        })?;
...
impl BlockHeader {
    ...
    pub fn from_bytes(buf: &[u8]) -> Result<Self> {
        if buf.len() < 4 {
            return Err(ToonDBError::InvalidData(
        ...
        if magic == V1_MAGIC.as_slice() {
            ...
            Err(ToonDBError::InvalidData(format!(
                "Unknown block magic: {:?}",
                magic
            )))
        }
...
impl MigratableBlock {
    ...
    pub fn verify_checksum(&self) -> Result<()> {
        ...
        if computed != stored {
            return Err(ToonDBError::DataCorruption {
                details: format!(
