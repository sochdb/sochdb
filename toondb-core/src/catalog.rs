...
//! - Built-in, TOON-QL, or external implementations
//! - Token savings: ~60% via TOON responses

use crate::toon::ToonSchema;
...
pub enum OperationImpl {
    ...
    BuiltIn(String),
    /// Stored procedure as TOON-QL query
    ToonQL(String),
...
pub struct CatalogEntry {
    ...
    pub schema: Option<ToonSchema>,
    /// Input schema (for operations)
    pub input_schema: Option<ToonSchema>,
    /// Output schema (for operations)
    pub output_schema: Option<ToonSchema>,
...
impl CatalogEntry {
    pub fn new_table(name: impl Into<String>, schema: ToonSchema, root_id: u64) -> Self {
        let now = now_micros();
        Self {
            name: name.into(),
    ...
    pub fn new_operation(
        name: impl Into<String>,
        input_schema: ToonSchema,
        output_schema: ToonSchema,
...
pub struct McpToolDescriptor {
    ...
    pub input_schema: ToonSchema,
    /// Output result schema
    pub output_schema: ToonSchema,
}
...
impl Catalog {
    ...
    /// Create a new table
    pub fn create_table(&mut self, schema: ToonSchema, root_id: u64) -> Result<(), String> {
    ...
    pub fn to_toon(&self) -> String {
        ...
        lines.push(format!(
            "toon_catalog[{}]{{name,type,fields,root,rows}}:",
            self.entries.len()
        ));
...
mod tests {
    ...
    fn test_auto_increment() {
        ...
        let schema = ToonSchema::new("users").field("id", ToonType::UInt);
        catalog.create_table(schema, 1).unwrap();
