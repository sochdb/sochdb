...
impl BatchedWalWriter {
    ...
    pub fn flush(&mut self) -> Result<usize> {
        ...
        let written = self.file.write_vectored(&iovecs).map_err(ToonDBError::Io)?;

        if written != expected {
            return Err(ToonDBError::Io(io::Error::new(
    ...
    pub fn sync(&mut self) -> Result<()> {
        ...
        self.file.sync_data().map_err(ToonDBError::Io)?;
...
impl BatchedWalReader {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self> {
        let file = File::open(path.as_ref()).map_err(ToonDBError::Io)?;
    ...
    pub fn read_batch(&mut self) -> Result<Option<Vec<TxnWalEntry>>> {
        ...
        match self.file.read_exact(&mut header) {
            ...
            Err(e) => return Err(ToonDBError::Io(e)),
        }

        // Validate magic
        let magic = u32::from_le_bytes(header[0..4].try_into().unwrap());
        if magic != BATCH_MAGIC {
            return Err(ToonDBError::Internal("Invalid batch magic".into()));
        }
        ...
        if stored_checksum != computed_checksum {
            return Err(ToonDBError::Internal(
        ...
        self.file.read_exact(&mut data).map_err(ToonDBError::Io)?;
