...
/// ToonDB magic number for SSTable files
pub const TOONDB_MAGIC: u64 = 0x544F4F4E44420001; // "TOONDB" + version 1
...
impl FileValidator {
    ...
    fn validate_impl(&self, path: &Path) -> std::result::Result<FileMetadata, ValidationError> {
        ...
        if magic != TOONDB_MAGIC {
            return Err(ValidationError::BadMagic {
                expected: TOONDB_MAGIC,
                actual: magic,
            });
        }
...
mod tests {
    ...
    fn create_valid_test_file() -> NamedTempFile {
        ...
        // Write header
        let mut header = [0u8; HEADER_SIZE];
        LittleEndian::write_u64(&mut header[0..8], TOONDB_MAGIC);
