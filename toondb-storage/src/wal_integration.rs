...
impl WalStorageManager {
    ...
    pub fn write(&self, txn_id: u64, key: Vec<u8>, value: Vec<u8>) -> Result<()> {
        ...
        let txn = txns
            .get_mut(&txn_id)
            .ok_or_else(|| ToonDBError::InvalidArgument("Transaction not found".into()))?;
...
impl MvccTransactionManager {
    ...
    pub fn read(&self, txn_id: u64, key: &[u8]) -> Result<Option<Vec<u8>>> {
        ...
        if isolation == IsolationLevel::Serializable {
            self.ssi_manager
                ...
                .map_err(|e| ToonDBError::Internal(format!("SSI conflict: {}", e.message)))?;
        }
    ...
    pub fn commit(&self, txn_id: u64) -> Result<u64> {
        ...
        let commit_ts = self
            ...
            .map_err(|e| ToonDBError::Internal(format!("Group commit error: {}", e)))?;
