...
//! ## Problem
//!
//! Current implementation stores each row as HashMap<String, ToonValue>, which incurs:
//! - 48 bytes minimum HashMap overhead per row
//! - 24 bytes per String key (pointer + length + capacity)
//! - 16-32 bytes per ToonValue (enum tag + union)
...
//! | 4 ToonValues | 128       | 32 (raw bytes) | 75%     |
