...
impl ColumnarMemtable {
    ...
    pub fn insert(&self, row_id: RowId, values: &[Option<&[u8]>]) -> Result<()> {
        if values.len() != self.schema.columns.len() {
            return Err(ToonDBError::InvalidData(format!(
...
use toondb_core::learned_index::LearnedSparseIndex;
...
impl ColumnGroup {
    ...
    pub fn from_memtable(
        ...
        let offsets_bytes = bincode::serialize(&column_offsets)
            .map_err(|e| ToonDBError::Serialization(e.to_string()))?;
        writer.write_u64::<LittleEndian>(offsets_bytes.len() as u64)?;
        writer.write_all(&offsets_bytes)?;

        // Serialize LSI
        let lsi_bytes =
            bincode::serialize(&lsi).map_err(|e| ToonDBError::Serialization(e.to_string()))?;
    ...
    pub fn open(path: PathBuf, schema: TableSchema, level: u32, sequence: u64) -> Result<Self> {
        ...
        if file_len < 12 {
            ...
            return Err(ToonDBError::Corruption("File too short".to_string()));
        }
        ...
        if magic != Self::MAGIC {
            return Err(ToonDBError::Corruption("Invalid magic bytes".to_string()));
        }
...
impl Lscs {
    ...
    pub fn mark_deleted(&self, row_id: RowId, _caller_txn_id: u64, txn_end: u64) -> Result<()> {
        ...
        let txn_end_idx = self
            ...
            .ok_or_else(|| {
                ToonDBError::InvalidData("Schema missing __txn_end column for MVCC".to_string())
            })?;

        // Get current row values
        let current = self
            .get(row_id)?
            .ok_or_else(|| ToonDBError::NotFound(format!("Row {} not found", row_id)))?;
    ...
    pub fn get(&self, row_id: RowId) -> Result<Option<Vec<Option<Vec<u8>>>>> {
        ...
        {
            use toondb_core::learned_index::LookupResult;
